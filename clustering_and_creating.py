...
def get_read_SV_coord(read, start, stop, buffer=200):
    ...
    for (cigarCode, cigarLen) in read.cigartuples:
        if sv_pos > 0:
            ...
            break
    deletion_read_stop = r_pos

    if deletion_read_start > deletion_read_stop:
        print('something went wrong here, the interval ends before it starts, return naive positions instead')
        print(read.query_name, deletion_read_start, deletion_read_stop, "vs",
              sv_pos + buffer + read.query_alignment_start, stop - read.reference_start + read.query_alignment_start)
...
def write_fasta(bam, cont, start, stop, path, buffer=10000, idx=''):
    ...
    for read in bam.fetch(contig=cont, start=start, stop=stop + 1):
        ...
        try:
            ...
            print(cont, start, 'for this read, no sequence is found:', read.query_name, )
    ...
    if len(seqs_h1) < 10 or len(seqs_h2) < 10:
        ...
        # there might be an error here, -1 0 1 , 0 1 also okay only 0 not okay
        # sum can be 0 or 1, len must be at least 2
        if sum(np.unique(cluster.labels_)) in [0, 1] and len(np.unique(cluster.labels_)) > 1:
            seqs_cl0 = []
            seqs_cl1 = []
